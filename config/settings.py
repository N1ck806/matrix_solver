...
# Render автоматически прокидывает RENDER_EXTERNAL_HOSTNAME
_render_host = os.environ.get("RENDER_EXTERNAL_HOSTNAME")
if _render_host:
    _extra_hosts.append(_render_host)

# На проде, если явно не задан DJANGO_ALLOWED_HOSTS, разрешаем всё —
# иначе первый деплой падает с DisallowedHost.
_default_hosts = (
...
# --- CSRF ---
# Django НЕ поддерживает wildcard-поддомены в CSRF_TRUSTED_ORIGINS.
# Указывайте точные origin'ы (схема + хост + опционально порт).
_csrf_default: list[str] = [
    "http://localhost",
    "http://localhost:8000",
    "http://127.0.0.1",
    "http://127.0.0.1:8000",
...
# На проде добавляем SITE_URL (если задан) в доверенные origin'ы.
_site_url_for_csrf = os.environ.get("SITE_URL", "").rstrip("/")
if _site_url_for_csrf and _site_url_for_csrf not in _csrf_default:
    _csrf_default.append(_site_url_for_csrf)
...
TEMPLATES: list[dict] = [
    {
        ...
        "OPTIONS": {
            ...
            "builtins": [
                ...
                # Свой тег для иконок: {% icon "name" size=16 %}
                "matrix_app.templatetags.matrix_icons",
            ],
            "debug": DEBUG,
            # В проде "" — иначе в шаблонах будут видны заглушки
            # "‹не найдено: ...›".
...
# Preconnect-хосты для <link rel="preconnect">
CDN_PRECONNECT: list[str] = [
...
# В DEBUG-режиме удобно логировать все SQL-запросы.
if DEBUG:
